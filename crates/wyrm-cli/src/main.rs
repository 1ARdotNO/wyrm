...
enum Command {
    ...
    Init {
        /// Source to read (repeatable — multiple sources combine into one model,
        /// aligned by environment). Default: auto-detect compose in the cwd.
        #[arg(long)]
        from: Vec<PathBuf>,
...
fn cmd_init(
    ...
    // Build each source and combine into one model. Terraform inline `# wyrm:`
    // annotations apply per source (they reference that source's own text).
    let mut acc: Option<otm_core::model::Otm> = None;
    for source in &sources {
        let text = read_source(source)?;
        let mut m = build_model(source, &text, &project)?;
        if looks_like_terraform(&text) {
            otm_core::enrich::enrich_inline(&mut m, &text).map_err(|e| e.to_string())?;
        }
        acc = Some(match acc {
            Some(prev) => otm_core::generate::combine(prev, m),
            None => m,
        });
    }
    ...
    otm_core::environ::link_by_environment(&mut otm, &env_cfg);
    ...
    let body = otm_core::to_yaml(&otm).map_err(|e| e.to_string())?;
    let doc = format!(
        "# Baseline threat model generated by `wyrm init` from {sources_label}.\n\
...
fn build_model(source: &Path, text: &str, project: &str) -> Result<otm_core::model::Otm, String> {
    ...
    if g::looks_like_tfplan(text) {
        ...
        let mut m = g::from_tfplan_json(text, project).map_err(|e| e.to_string())?;
        g::stamp_source(&mut m, "terraform");
        return Ok(m);
    }
            ...
            || source.join("kustomization.yml").is_file())
    {
        ...
        let mut m = g::from_manifests(&rendered, project).map_err(|e| e.to_string())?;
        g::stamp_source(&mut m, "kubernetes");
        return Ok(m);
    }
    ...
    let build_tf = |docs: &[(String, String)]| {
        let (mut otm, skipped) = g::from_terraform_scoped(docs, project);
        if skipped > 0 {
            eprintln!(
                "note: skipped {skipped} unparseable .tf file(s) (placeholders / unsupported HCL)."
            );
        }
        g::stamp_source(&mut otm, "terraform");
        otm
    };
    let build_k8s = |text: &str| -> Result<otm_core::model::Otm, String> {
        let mut m = g::from_manifests(text, project).map_err(|e| e.to_string())?;
        g::stamp_source(&mut m, "kubernetes");
        Ok(m)
    };
    ...
    match (has_tf, k8s_text) {
        (true, Some(k8s)) => {
            ...
            let mut k8s = build_k8s(&k8s)?;
            g::place_in_cluster(&tf, &mut k8s);
            Ok(g::combine(tf, k8s))
        }
        (true, None) => Ok(build_tf(&tf_docs)),
        (false, Some(k8s)) => build_k8s(&k8s),
        (false, None) => {
            let mut m = g::from_compose(text, project).map_err(|e| e.to_string())?;
            g::stamp_source(&mut m, "compose");
            Ok(m)
        }
    }
}
